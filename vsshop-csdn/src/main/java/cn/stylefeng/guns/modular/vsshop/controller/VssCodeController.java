...
public class VssCodeController extends BaseController {
    ...
    public Object list(VssCode vssCode) {
        ...
        if(StrUtil.isNotBlank(vssCode.getBeginTime())){
            entityWrapper.ge("update_time",vssCode.getBeginTime());
        }
        if(StrUtil.isNotBlank(vssCode.getEndTime())){
            entityWrapper.le("update_time",vssCode.getEndTime());
        }
        if(StrUtil.isNotBlank(vssCode.getCode())){
            entityWrapper.eq("code",vssCode.getCode());
        }
        if(StrUtil.isNotBlank(vssCode.getStatus())){
            entityWrapper.eq("status",vssCode.getStatus());
        }
        if(StrUtil.isNotBlank(vssCode.getOwner())){
            entityWrapper.eq("owner",vssCode.getOwner());
        }
    ...
    public void detailListExport(VssCode vssCodeQuery,ModelMap map, HttpServletRequest request, HttpServletResponse response)  {
        ...
        try {
            ...
            if(StrUtil.isNotBlank(vssCodeQuery.getStatus())){
            ...
            vssCodeListTemp.forEach(vssCode -> {
                ...
                vssCode.setCode(String.format("http://%s/down?code=%s",ossHelper.getWebDomain(), vssCode.getCode()));
            });
