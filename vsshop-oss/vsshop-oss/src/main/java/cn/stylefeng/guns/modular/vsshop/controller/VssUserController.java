...
public class VssUserController extends BaseController {
    ...
    public Object list(String condition) {
        ...
        entityWrapper.orderBy("insert_time",true);
        ...
        if(CollUtil.isNotEmpty(vssUserList)){
            vssUserList.forEach(vssUser -> {
                try {
                    VssIp vssIp=vssIpService.selectById(vssUser.getIpUuid());
                    VssRegion vssRegion=vssRegionService.selectById(vssIp.getRegionUuid());
                    vssUser.setIpUuid(vssRegion.getName()+":"+ vssIp.getIp());
                }catch (Exception e){
